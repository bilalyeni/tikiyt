...
client.on('modalSubmit', async (modal) => {
  ...
  const logs = db.get(`ticketlogs_${modal.guild.id}`) || null;
    ...
    .setTitle(`${client.emoji.manage} | TICKET CLOSED`)
          .setColor(`#2b2d30`)
          .addField(`**OPENED BY:**`, `<@${tcopener.user.id}>\`\`\`\n${tcopener.user.username}(${tcopener.user.id})\n\`\`\``)
    .addField(`**CLOSED BY:**`, `<@${modal.user.id}>\`\`\`\n${modal.user.username}(${modal.user.id})\n\`\`\``)
    .addField(`**REASON:**`, `${Formatters.codeBlock('markdown', firstResponse) || "`NO REASON ADDED`"}`)
    .setFooter(`YSN Group™ Ticket Systems`)
