...
if(!interaction.isButton() && !interaction.isSelectMenu()) return;
      ...
      if(interaction.customId == `create_ticket${s}`) {
        ...
        interaction.guild.channels.create(`${tcname || `ticket-${interaction.user.username}`}`, {
                    ...
                    type: 'text',
                    parent: cat,
                    topic: `📨 Community Team ticket for: ${interaction.user.tag} (${interaction.user.id})`
                }).catch(() => {
        ...
        .setAuthor(`Community Team ticket for: ${interaction.user.tag}`, interaction.user.displayAvatarURL(), `https://discord.gg/ysngroup`)
